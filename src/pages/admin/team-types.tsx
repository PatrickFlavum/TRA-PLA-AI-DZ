...
import { loadTeamTypes, createTeamType, updateTeamType, deleteTeamType } from '@/lib/supabase'
...
export default function TeamTypesPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await createTeamType(newName.trim(), newColor)
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      await updateTeamType(id, { name: editName.trim(), color: editColor, sort_order: sortOrderNum })
      setEditId(null); loadData()
    } catch { setError('Fehler beim Aktualisieren.') }
  }

  const handleDelete = async (t: TeamType) => {
    if (!confirm(`Team-Typ «${t.name}» löschen?`)) return
    try { await deleteTeamType(t.id); loadData() }
    catch { setError('Fehler beim Löschen. Möglicherweise wird der Typ noch verwendet.') }
  }

  return (
    <>
      <Head><title>Team-Typen – AI@DZ</title></Head>
      <AdminLayout title="Team-Typen">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Team-Typen</h1>
        <p className="text-xs text-gray-400 mb-6">Team-Typen werden Teams zugewiesen und als farbige Labels angezeigt.</p>

        <form onSubmit={handleCreate} className="bg-white rounded-xl border border-gray-200 p-4 mb-6 flex gap-3 items-end">
          <div className="flex-1">
            <label htmlFor="tt-name" className="block text-xs font-medium text-gray-600 mb-1">Neuer Team-Typ</label>
            <input id="tt-name" type="text" value={newName} onChange={e => setNewName(e.target.value)}
              placeholder="z.B. Pilot-Team"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </div>
          <div className="shrink-0">
            <label htmlFor="tt-color" className="block text-xs font-medium text-gray-600 mb-1">Farbe</label>
            <div className="flex items-center gap-2">
              <input id="tt-color" type="color" value={newColor} onChange={e => setNewColor(e.target.value)}
        ...
        {loading ? <p className="text-gray-400 text-sm">Lädt…</p> : types.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
            <p className="text-gray-500 text-sm">Noch keine Team-Typen vorhanden.</p>
          </div>
        ) : (
                ...
                {editId === t.id ? (
                  <div className="flex gap-2 items-center flex-1">
                    ...
                    <input type="text" value={editColor} onChange={e => setEditColor(e.target.value)} maxLength={7}
                      ...
                      aria-label="Farbe als Hex-Code" />
                    <button type="button" onClick={() => handleUpdate(t.id)} className="text-sm text-brand-600 hover:text-brand-700 font-medium">Speichern</button>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="w-6 text-center text-xs text-gray-400 font-mono shrink-0">{t.sort_order}</span>
                      <div className="w-5 h-5 rounded shrink-0" style={{ backgroundColor: t.color ?? DEFAULT_COLOR }} />
                    ...
                    <div className="flex gap-3">
                      <button type="button" onClick={() => { setEditId(t.id); setEditName(t.name); setEditColor(t.color ?? DEFAULT_COLOR); setEditSortOrder(String(t.sort_order)) }}
